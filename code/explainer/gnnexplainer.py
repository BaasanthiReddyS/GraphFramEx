...
class GNNExplainer(torch.nn.Module):
    ...
    def __init_masks__(self, x, edge_index):
        ...
        self.node_feat_mask = torch.nn.Parameter(
            torch.randn(num_features, device=self.device) * std_node)
        std_edge = torch.nn.init.calculate_gain('relu') * sqrt(2.0 / (2 * num_nodes))
        self.edge_mask = torch.nn.Parameter(
            torch.randn(num_edges, device=self.device) * std_edge)
    ...
    def explain_node(self, node_idx, x, edge_index, target=None, **kwargs):
        ...
        x_sub, edge_index_sub, mapping, edge_mask_bool, subset, kwargs_sub =             self.__subgraph__(node_idx, x, edge_index, **kwargs)
        ...
        # ── expand subgraph edge mask back to full graph size ──
