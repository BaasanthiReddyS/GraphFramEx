...
for seed in [42, 43, 44]:
    model = GCN(num_node_features=1433, hidden_dim=16, num_classes=7, dropout=0.5, num_layers=2, device=device)
    ...
    for rate in [0.10, 0.15, 0.20]:
        ...
        mask = (torch.rand_like(data.x) > rate).float()
        x_masked = data.x * mask
        with torch.no_grad():
            out_n = model(x_masked, data.edge_index, edge_weight=data.edge_weight)
        ...
        print(f"Seed {seed} | {int(rate*100)}% | Edge drop: {clean_acc-acc_e:.4f} | Node mask drop: {clean_acc-acc_n:.4f}")

    results[f"seed_{seed}"] = {"clean_acc": round(clean_acc, 4), "ood": ood_results}

with open("/kaggle/working/ood_results_full.json", "w") as f:
    json.dump(results, f, indent=2)
print("Saved to /kaggle/working/ood_results_full.json")
